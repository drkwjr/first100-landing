...
Extracted credit parsing logic to its own function.
...
### Verification notes (required for non-trivial changes)

If a change touches TypeScript/React, core business logic, auth/billing, or deployment paths, include a short verification block in the commit body:

- Commands run (e.g. `cd frontend && npm run quality`, `cd backend && npm run build`, `yarn tsc -b`)
- What was manually verified (especially UI flows)
- If any gate was not run, state why
