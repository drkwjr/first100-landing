...
## 🏗️ Architecture Discipline (Avoid Monoliths)

Code style isn’t just formatting — it’s how we keep the system maintainable.

- Prefer **small modules with clear boundaries** over “do-everything” files.
- If a file is trending toward a “god file,” stop and refactor into smaller modules.
- Keep functions cohesive: if a function has multiple responsibilities, split it.
- When introducing a new subsystem, prefer an explicit module boundary (directory + types + tests) rather than scattering helpers.

See also:
- `devbrain-handbook/refactor-rules.md`
- `devbrain-handbook/architecture-ops.md` (to be added)
...
### Build vs buy (don’t reinvent by default)

If a problem is already solved well by a reputable library or existing internal module, prefer **composing** it rather than designing from scratch. New abstractions must justify their existence.
