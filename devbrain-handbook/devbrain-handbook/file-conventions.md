...
```
/
├── src/              → App logic only (single-package projects)
├── frontend/         → Frontend package (monorepo projects)
├── backend/          → Backend package (monorepo projects)
├── shared/           → Shared types/utilities (monorepo projects)
├── docs/             → Product + architecture docs (repo-level)
├── devbrain-handbook/→ Dev Brain standards (repo-level)
├── prompts/          → Prompt templates used in Cursor or agents
├── scripts/          → Local utilities, agent runners
├── .devbrainrc       → Dev Brain config (agents, triggers)
├── mcp.json          → Local MCP inventory (ignored by git)
```
