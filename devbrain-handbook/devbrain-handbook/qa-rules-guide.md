...
QA rules often validate **external data** (generated rules, scraped sources, pipeline output). Treat inbound data as untrusted:

- Start from `unknown` at the boundary and validate required shape.
- Prefer using shared helpers (e.g. `assertRequired`, `assertEnum`) and fail with clear, actionable messages.
...
After changing QA rules, prefer these verification steps:

- `npm run qa:rules -- --calculator <id> --state <STATE>` (spot-check)
- `npm run validate:rules-files` (repo-wide rule file validation)
- `cd backend && npm run build` (ensures TS compilation for pipeline code)
