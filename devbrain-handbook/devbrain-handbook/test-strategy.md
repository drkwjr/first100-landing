...
- **Lint & TypeScript Standards**: See [`lint-and-typescript-style.md`](./lint-and-typescript-style.md) for rules on test code (relaxed `any` usage, etc.)
- **Coding Style**: See [`coding-style.md`](./coding-style.md) for general code conventions
...
## Repo-specific commands (DocketMath)

This repo is a monorepo (`frontend/`, `backend/`). Preferred commands:

### Fast path (tight iteration)

- Frontend: `cd frontend && npm run quality`
- Backend: `cd backend && npm run lint && npm run build`

### Full path (pre-merge confidence)

- Root: `yarn lint`
- Root: `yarn tsc -b`
- CI script: `yarn ci-check`

### Dev Brain wrappers (no-hang, progress output)

- `yarn devbrain:quality`
- `yarn devbrain:verify:frontend`
